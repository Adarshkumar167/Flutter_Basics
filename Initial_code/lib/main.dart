...
class CounterStateVaild extends CounterState {
  const CounterStateVaild(super.value);
}
...
class CounterStateInvalidNumber extends CounterState {
  ...
  const CounterStateInvalidNumber({
    ...
    required int previouseValue,
  }) : super(previouseValue);
}
...
class CounterBloc extends Bloc<CounterEvent, CounterState> {
  CounterBloc() : super(const CounterStateVaild(0)) {
    on<IncrementEvent>((event, emit) {
      final integer = int.tryParse(event.value);
      if (integer == null) {
        emit(CounterStateInvalidNumber(
          invalidValue: event.value,
          previouseValue: state.value,
        ));
      } else {
        emit(CounterStateVaild(state.value + integer));
      }
    });
